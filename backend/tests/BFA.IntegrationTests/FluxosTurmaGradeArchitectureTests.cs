...
public sealed class FluxosTurmaGradeArchitectureTests
{
    ...
    public void V013_permanece_intacta_e_v014_e_somente_corretiva()
    {
        ...
        Assert.True(files.Length == 26, $"Esperadas 26 migrations, encontradas {files.Length}.");
        Assert.Contains("V024__adicionar_auditoria_cancelamento_aula.sql", files);
        Assert.Contains("V025__criar_day_use.sql", files);
        Assert.Contains("V026__permitir_exclusao_day_use.sql", files);
